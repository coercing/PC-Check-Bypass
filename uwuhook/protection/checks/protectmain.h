...
enum class ProtectionStage {
    STAGE_NONE = 0,
    STAGE_INITIAL = 1,      // Basic protections (anti-dump, thread hiding)
    STAGE_CONTINUOUS = 2,   // Continuous monitoring (anti-debug, process scanning)
    STAGE_FULL = 3          // Full protection (everything)
};

// declarations
extern void mainprotect(ProtectionStage stage = ProtectionStage::STAGE_FULL);
extern void initialize_basic_protections();
extern void start_continuous_protection();
extern void stop_continuous_protection();
extern void enable_critical_protections();
extern void disable_protections();
extern ProtectionStage get_current_protection_stage();